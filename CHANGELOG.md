...
- Raised the Doctrine Collections baseline to 2.0, with support for both 2.x and 3.x.
...
The dependency baseline changes are breaking: version 5.0.0 requires PHP >= 8.2, Symfony >= 6.4 and Doctrine Collections >= 2.0. The historical `exit()` behaviour when lock acquisition or PHP binary discovery fails remains unchanged for backwards compatibility.
